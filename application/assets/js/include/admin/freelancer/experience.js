$(function(){
    ...
    function loadExperiences() {
      $.ajax({
        url: formUrl('admin/freelancer/experience', {freelancer_id:freelancer_id}),
        type: 'post',
        dataType: 'json',
        beforeSend: function() {
    ...
    $('#add-experience').click(function(e){
      ...
      $('#formExperience').attr('action', formUrl('admin/freelancer/experience/add', {freelancer_id:freelancer_id}));
    ...
    $(document).on('click', '.edit-experience', function(e){
      ...
      if(cweid != '' && typeof(cweid) != 'undefined'){
        $.ajax({
          url: formUrl('admin/freelancer/experience/detail/' + cweid, {freelancer_id:freelancer_id}),
          type: 'post',
          data: {view: 1},
          dataType: 'json',
          beforeSend: function() {
          ...
          success: function(res) {
            if(res.success) {
              ...
              $('#formExperience').attr('action', formUrl('admin/freelancer/experience/edit/' + cweid, {freelancer_id:freelancer_id}));
              ...
              $('#formExperience').find('.experience-start-date').val(date_format(category.cwe_start_date));
              if(category.cwe_current_company == 1){
                $('#experience-end-date-row').hide();
              } else {
                $('#experience-end-date-row').show();
              }
              $('#formExperience').find('.experience-end-date').val(date_format(category.cwe_end_date));
              $('.selectpicker').selectpicker('refresh');
            } else if(res.error) {
                $.ALERT.show('danger', res.message);
            } else {
    ...
    $(document).on('click', '.delete-experience', function(e){
      ...
      $('#formDeleteExperience').submit(function(e){
        ...
        var button_txt = $form.find('button[type=submit]').html();
        $.ajax({
          url: formUrl('admin/freelancer/experience/delete/' + cweid, {freelancer_id:freelancer_id}),
          type: 'post',
          dataType: 'json',
          beforeSend: function() {
            $form.find('button[type=submit]').html(loading_txt).attr('disabled', true);
