...
function populateCountries(countryElementId, stateElementId, selectedCountry, selectedState){
	...
	countryElement.selectedIndex =   document.getElementById( countryElementId ).selectedIndex;;
	for (var i=0; i<country_arr.length; i++) {
		countryElement.options[countryElement.length] = new Option(country_arr[i],country_arr[i]);
		if (countryElement.options[i].value == selectedCountry) {
	...
	var selectedCountryIndex = document.getElementById( countryElementId ).selectedIndex;
	if(selectedCountryIndex && stateElementId ){
		var stateElement = document.getElementById( stateElementId );
		...
		for (var i=0; i<state_arr.length; i++) {
			stateElement.options[stateElement.length] = new Option(state_arr[i],state_arr[i]);
			if (stateElement.options[i].value == selectedState) {
...
function searchStateByCountry(countryName, str){
    var searchArray = []; var limit=5; var sl = 0;
    var countryIndex = country_arr.indexOf(countryName);
    var state_arr = s_a[countryIndex].split("|");
    if(state_arr.length > 0){
        sl = 0;
        for (var si=0; si<state_arr.length; si++) {
            if (sl <= limit){
                if(state_arr[si].match(str)){
                    searchArray.push(state_arr[si]);
                    sl++;
                }
            } else {
                break;
