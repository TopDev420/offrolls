		...
		if($this->valid){
			...
			if($experiences) {
                foreach($experiences as $experience){
                    $start_date = json_decode($experience->cwe_start_date);
                    if($experience->cwe_start_date ){
                    	$start_date = json_decode($experience->cwe_start_date);
                    	$startDate = json_decode(json_encode(array(
                    		'month' =>  getMonthByKey($start_date->month),
                    		'year' => $start_date->month
                    	)));
                    } else {
                    ...
                    if($experience->cwe_end_date ){
                    	$end_date = json_decode($experience->cwe_end_date);
                    	$endDate = json_decode(json_encode(array(
                    		'month' =>  getMonthByKey($end_date->month),
                    		'year' => $end_date->month
                    	)));
                    } else {
                    	$end_date = $endDate = '';
