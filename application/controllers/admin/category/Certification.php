...
class Certification extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        $this->category_type_id = CERTIFICATION_TYPE;
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Certification';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Certification',
			'href' => base_url() . 'admin/certification'
		);
		$data['active_menu'] = 'mnu-certification';
		...
		$config['base_url'] = base_url() . 'admin/category/certification/index/';
		...
		$this->load->view('admin/category/certification');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Certification already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Certification Added';
			} else {
				$json['error'] = 'Certification not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Certification information modified';
						} else {
							$json['error'] = 'Certification not exist';
						}
					} else {
						$json['error'] = 'Certification not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Certification Deleted Successfully';
				} else {
					$json['error'] = 'Certification Not Deleted!';
				}
			} else {
