...
class Jobspecialization extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        $this->category_type_id = JOB_SPECIALIZATION_TYPE;
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Job Specialization';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			...
			'href' => base_url() . 'admin/jobspecialization'
		);
		$data['active_menu'] = 'mnu-jobspecialization';
		...
		$config['base_url'] = base_url() . 'admin/category/jobspecialization/index/';
		...
		$this->load->view('admin/category/job_specialization');
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Category information modified';
						} else {
							$json['error'] = 'Category not exist';
						}
					} else {
						$json['error'] = 'Category not exist';
					}
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					...
					$json['error'] = 'Category Not Deleted!';
				}
			} else {
