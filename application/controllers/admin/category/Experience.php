...
class Experience extends CI_Controller {
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Experience';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Experience',
			'href' => base_url() . 'admin/experience'
		);
		$data['active_menu'] = 'mnu-experience';
		...
		$config['base_url'] = base_url() . 'admin/category/experience/index/';
		...
		$this->load->view('admin/category/experience');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Experience Name already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Experience Added';
			} else {
				$json['error'] = 'Experience not added!';
			}
		}
	...
	public function edit($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			if($this->input->post('view') == 1){
				...
				if($exist) {
					$json['error'] = 'Experience already exist';
				} else {
					$category = $this->model_jobcategory->getCategory($category_id);
					if($category) {
						$result = $this->model_jobcategory->editCategory($category_id);
						if($category) {
							$json['success'] = 'Experience information modified';
						} else {
							$json['error'] = 'Experience not exist';
						}
					} else {
						$json['error'] = 'Experience not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Experience Deleted Successfully';
				} else {
					$json['error'] = 'Experience Not Deleted!';
				}
			} else {
				$json['error'] = 'Experience Not Found';
			}
		} else {
