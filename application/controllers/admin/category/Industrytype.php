...
class Industrytype extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        $this->category_type_id = COMPANY_CATEGORY_TYPE;
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Industry Type';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Industry Type',
			'href' => base_url() . 'admin/Industrytype'
		);
		$data['active_menu'] = 'mnu-industrytype';
		...
		$config['base_url'] = base_url() . 'admin/category/industrytype/index/';
		...
		$this->load->view('admin/category/industry_type');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Job type already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = ' Added';
			} else {
				$json['error'] = 'Industry type not added!';
			}
		}
	...
	public function edit($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			if($this->input->post('view') == 1){
				...
				if($exist) {
					$json['error'] = 'Industry type already exist';
				} else {
					$category = $this->model_jobcategory->getCategory($category_id);
					if($category) {
						$result = $this->model_jobcategory->editCategory($category_id);
						if($category) {
							$json['success'] = 'Industry type information modified';
						} else {
							$json['error'] = 'Industry type not exist';
						}
					} else {
						$json['error'] = 'Industry type not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Industry type Deleted Successfully';
				} else {
					$json['error'] = 'Industry type Not Deleted!';
				}
			} else {
				$json['error'] = 'Industry type Not Found';
			}
		} else {
