...
class Noticeperiod extends CI_Controller {
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Notice Period';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Notice Period',
			'href' => base_url() . 'admin/noticeperiod'
		);
		$data['active_menu'] = 'mnu-notice-period';
		...
		$config['base_url'] = base_url() . 'admin/category/noticeperiod/index/';
		...
		$this->load->view('admin/category/notice_period');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Notice period already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Notice period Added';
			} else {
				$json['error'] = 'Notice period not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Notice period information modified';
						} else {
							$json['error'] = 'Notice period not exist';
						}
					} else {
						$json['error'] = 'Notice period not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Notice period Deleted Successfully';
				} else {
					$json['error'] = 'Notice period Not Deleted!';
				}
			} else {
				$json['error'] = 'Notice period Not Found';
			}
		} else {
