...
class Technology extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        $this->category_type_id = TECHNOLOGY_TYPE;
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Technology';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Technology',
			'href' => base_url() . 'admin/technology'
		);
		$data['active_menu'] = 'mnu-technology';
		...
		$config['base_url'] = base_url() . 'admin/category/technology/index/';
		...
		$this->load->view('admin/category/technology');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Technology already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Technology Added';
			} else {
				$json['error'] = 'Technology not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Technology information modified';
						} else {
							$json['error'] = 'Technology not exist';
						}
					} else {
						$json['error'] = 'Technology not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Technology Deleted Successfully';
				} else {
					$json['error'] = 'Technology Not Deleted!';
				}
			} else {
				$json['error'] = 'Technology Not Found';
			}
		} else {
