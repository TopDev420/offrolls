...
class Keyword extends CI_Controller {
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Keyword';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Keyword',
			'href' => base_url() . 'admin/keyword'
		);
		$data['active_menu'] = 'mnu-keyword';
		...
		$config['base_url'] = base_url() . 'admin/category/keyword/index/';
		...
		$this->load->view('admin/category/keyword');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Keyword already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Keyword Added';
			} else {
				$json['error'] = 'Keyword not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Keyword information modified';
						} else {
							$json['error'] = 'Keyword not exist';
						}
					} else {
						$json['error'] = 'Keyword not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Keyword Deleted Successfully';
				} else {
					$json['error'] = 'Keyword Not Deleted!';
				}
			} else {
				$json['error'] = 'Keyword Not Found';
			}
		} else {
