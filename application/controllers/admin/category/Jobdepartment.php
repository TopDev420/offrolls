...
class Jobdepartment extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        $this->category_type_id = JOB_DEPARTMENT_TYPE;
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Job Category';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Job Category',
			'href' => base_url() . 'admin/jobdepartment'
		);
		$data['active_menu'] = 'mnu-jobdepartment';
		...
		$config['base_url'] = base_url() . 'admin/category/jobdepartment/index/';
		...
		$this->load->view('admin/category/job_department');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Job department already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Job department Added';
			} else {
				$json['error'] = 'Job department not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Job department modified';
						} else {
							$json['error'] = 'Job department not exist';
						}
					} else {
						$json['error'] = 'Job department not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Job department Deleted Successfully';
				} else {
					$json['error'] = 'Job department Not Deleted!';
				}
			} else {
				$json['error'] = 'Job department Not Found';
			}
		} else {
