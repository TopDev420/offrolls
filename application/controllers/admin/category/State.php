...
class State extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        $this->category_type_id = STATE_TYPE;
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'State';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'State',
			'href' => base_url() . 'admin/state'
		);
		$data['active_menu'] = 'mnu-state';
		...
		$config['base_url'] = base_url() . 'admin/category/state/index/';
		...
		$this->load->view('admin/category/state');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'State Name already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'State Added';
			} else {
				$json['error'] = 'State not added!';
			}
		}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'State Deleted Successfully';
				} else {
					$json['error'] = 'State Not Deleted!';
				}
			} else {
