...
class Jobdesignation extends CI_Controller {
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Job Designation ';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			...
			'href' => base_url() . 'admin/jobdesignation '
		);
		$data['active_menu'] = 'mnu-jobdesignation ';
		...
		$config['base_url'] = base_url() . 'admin/category/jobdesignation/index/';
		...
		$this->load->view('admin/category/job_designation');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Job designation already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Job designation Added';
			} else {
				$json['error'] = 'Job designation not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Job designation information modified';
						} else {
							$json['error'] = 'Job designation not exist';
						}
					} else {
						$json['error'] = 'Job designation not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Job designation Deleted Successfully';
				} else {
					$json['error'] = 'Job designation Not Deleted!';
				}
			} else {
				$json['error'] = 'Job designation Not Found';
			}
		} else {
