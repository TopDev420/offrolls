...
class Skills extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        $this->category_type_id = SKILLS_TYPE;
    ...
    public function index(){
		...
		$data['logged'] = true;
		$data['heading_title'] = 'Skills';	//Heading Title
		...
		$config['base_url'] = base_url() . 'admin/category/skills/index/';
		...
		$this->load->view('admin/category/skills');
	...
	public function add() {
		...
		if($category) {
			$json['error'] = 'Skill already exist';
		} else {
			$result = $this->model_jobcategory->addCategory($this->category_type_id);
			if($result) {
				$json['success'] = 'Skill Added';
			} else {
				$json['error'] = 'Skill not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Skill information modified';
						} else {
							$json['error'] = 'Skill not exist';
						}
					} else {
						$json['error'] = 'Skill not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Skill Deleted Successfully';
				} else {
					$json['error'] = 'Skill Not Deleted!';
				}
			} else {
