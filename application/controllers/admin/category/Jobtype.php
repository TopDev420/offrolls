...
class Jobtype extends CI_Controller {
    ...
    public function index(){
		...
		$data['heading_title'] = 'Job Type';	//Heading Title
		...
		$data['breadcrumb'][] = array(
			'name' => 'Job Type',
			'href' => base_url() . 'admin/jobtype'
		);
		$data['active_menu'] = 'mnu-jobtype';
		...
		$config['base_url'] = base_url() . 'admin/category/jobtype/index/';
		...
		$this->load->view('admin/category/job_type');
	...
	public function add() {
		...
		if($category) {
			...
			if($result) {
				$json['success'] = 'Job type Added';
			} else {
				$json['error'] = 'Job type not added!';
			}
		}
			...
			if($this->input->post('view') == 1){
				...
				if($exist) {
					...
					if($category) {
						...
						if($category) {
							$json['success'] = 'Job type information modified';
						} else {
							$json['error'] = 'Job type not exist';
						}
					} else {
						$json['error'] = 'Job type not exist';
					}
	...
	public function delete($category_id) {
		...
		if($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($category) {
				...
				if($delete) {
					$json['success'] = 'Job type Deleted Successfully';
				} else {
					$json['error'] = 'Job type Not Deleted!';
				}
			} else {
				$json['error'] = 'Job type Not Found';
			}
		} else {
